...
for (const item of window.ENACTPHYS_STATES) {
  ...
  function draw(){
    ...
    if(item.kind==='embedding'){
      ...
      if(background)ctx.drawImage(background,0,0);else {
      for(let d=0;d<512;d++){
      ...
      background=document.createElement('canvas');background.width=W;background.height=H;background.getContext('2d').drawImage(canvas,0,0);
      }
