...
function choose(c){switching=true;v.pause();active=c;pendingFrame=null;userPaused=reduced;v.removeAttribute('src');v.dataset.poster=base+c.id+'_poster.jpg';v.poster=v.dataset.poster;v.load();section.querySelector('#comparison-title').textContent=c.label;section.querySelector('#comparison-download').href=base+c.id+'_original_speed_comparison.mp4';for(const b of tabs.children)b.setAttribute('aria-pressed',String(b.dataset.id===c.id));if(near||visible)prepare();play();}
